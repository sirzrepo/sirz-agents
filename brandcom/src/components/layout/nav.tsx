...
export default function NavBar({ sidebarItems = [] }: NavBarProps) {
    ...
    // Generate menu items based on auth state and sidebar items
    const getMenuItems = (): MenuItem[] => {
      const items: MenuItem[] = [];
      
      // Add sidebar items to the menu on mobile
      if (window.innerWidth < 1024 && sidebarItems.length > 0) {
        sidebarItems.forEach((item) => {
          items.push({
            ...
            className: 'lg:hidden' // Only show these items on mobile
          });
        });
        
        // Add a divider between sidebar items and profile items
        items.push({
          icon: <div className="w-full h-px bg-gray-200 my-2" />,
          label: '',
          onClick: () => {},
          className: 'lg:hidden cursor-default pointer-events-none'
        });
      }
